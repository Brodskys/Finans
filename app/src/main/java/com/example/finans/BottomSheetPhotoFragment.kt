...
class BottomSheetPhotoFragment : BottomSheetDialogFragment() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        val parentFragment = parentFragmentManager.fragments.firstOrNull()

        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                REQUEST_PICK_PHOTO -> {
                    val imageUri = data?.data
                    imageViewModel.setGalleryImageUri(imageUri)

                    if (parentFragment is BottomSheetNewOperationFragment) {

                    } else if (parentFragment is BottomSheetSettingsFragment) {
                ...
                REQUEST_IMAGE_CAPTURE  -> {
                   val imageBitmap = data?.extras?.get("data") as? Bitmap
                    ...
                    imageViewModel.setCameraImageUri(imageUri)
